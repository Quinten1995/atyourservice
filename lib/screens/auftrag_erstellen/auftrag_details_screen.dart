...
class AuftragDetailsScreen extends StatefulWidget {
  ...
  const AuftragDetailsScreen({Key? key, required this.formData})
    : super(key: key);
...
class _AuftragDetailsScreenState extends State<AuftragDetailsScreen> {
  ...
  late final _titelController = TextEditingController(
    text: widget.formData.titel ?? '',
  );
  late final _beschreibungController = TextEditingController(
    text: widget.formData.beschreibung ?? '',
  );
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isSelected
                ? AuftragDetailsScreen.primaryColor
                : Colors.grey[300]!,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    title,
                    overflow: TextOverflow.ellipsis, // <- Overflow verhindern
                    maxLines: 1,
                    style: TextStyle(
          ...
          child: SingleChildScrollView(
            child: Padding(
              ...
              child: Card(
                color: Colors.white,
                elevation: 7,
                shadowColor: AuftragDetailsScreen.primaryColor.withOpacity(
                  0.09,
                ),
                              ...
                              child: SizedBox(
                                ...
                                child: ElevatedButton.icon(
                                  ...
                                  style: ElevatedButton.styleFrom(
                                    ...
                                    elevation: 4,
                                    shadowColor: AuftragDetailsScreen
                                        .primaryColor
                                        .withOpacity(0.20),
                                  ),
                                  onPressed: () {
                                    if (_formKey.currentState?.validate() ??
                                        false) {
                                      widget.formData.titel = _titelController
                                          .text
                                          .trim();
                                      widget.formData.beschreibung =
                                          _beschreibungController.text.trim();
                                      widget.formData.preisTyp =
                                          _selectedPreisTyp;

                                      final preisText = _preisController.text
                                          .trim();
                                      if (_selectedPreisTyp == "gesamt" ||
                                          _selectedPreisTyp == "stunden") {
                                        widget.formData.preis = double.tryParse(
                                          preisText.replaceAll(',', '.'),
                                        );
                                      } else {
                                        widget.formData.preis = null;
                                      }

                                      final hinweis = _preisHinweisController
                                          .text
                                          .trim();
                                      widget.formData.preisHinweis =
                                          hinweis.isEmpty ? null : hinweis;

                                      Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                          builder: (context) =>
                                              AuftragAdresseScreen(
